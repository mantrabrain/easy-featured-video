<?php
/**
 * Easy_Featured_Video Admin Settings Class
 *
...
if ( ! class_exists( 'Easy_Featured_Video_Admin_Settings', false ) ) :

	/**
	 * Easy_Featured_Video_Admin_Settings Class.
	 */
	class Easy_Featured_Video_Admin_Settings {
		...
		public static function get_settings_pages() {
			...
			if ( empty( self::$settings ) ) {
				...
				$settings[] = include 'settings/class-easy-featured-video-settings-general.php';


				self::$settings = apply_filters( 'easy_featured_video_get_settings_pages', $settings );
			}
		...
		public static function save() {
			...
			check_admin_referer( 'easy-featured-video-settings' );

 			// Trigger actions.
			do_action( 'easy_featured_video_settings_save_' . $current_tab );
			do_action( 'easy_featured_video_update_options_' . $current_tab );
			do_action( 'easy_featured_video_update_options' );

			self::add_message( __( 'Your settings have been saved.', 'easy-featured-video' ) );
			self::check_download_folder_protection();

			// Clear any unwanted data and flush rules.
			update_option( 'easy_featured_video_queue_flush_rewrite_rules', 'yes' );

			do_action( 'easy_featured_video_settings_saved' );
		}
		 ...
		 * Settings page.
		 *
		 * Handles the display of the main easy-featured-video settings page in admin.
		 */
		public static function output() {

			global $current_section, $current_tab;

			do_action( 'easy_featured_video_settings_start' );



			// Get tabs for the settings page.
			$tabs = apply_filters( 'easy_featured_video_settings_tabs_array', array() );
		 ...
		 * Output admin fields.
		 *
		 * Loops though the easy-featured-video options array and outputs each field.
		 *
			...
			foreach ( $options as $value ) {
				...
				switch ( $value['type'] ) {
					...
					case 'title':
						...
						if ( ! empty( $value['id'] ) ) {
							do_action( 'easy_featured_video_settings_' . sanitize_title( $value['id'] ) );
						}
						break;
					...
					case 'sectionend':
						if ( ! empty( $value['id'] ) ) {
							do_action( 'easy_featured_video_settings_' . sanitize_title( $value['id'] ) . '_end' );
						}
						echo '</table>';
						if ( ! empty( $value['id'] ) ) {
							do_action( 'easy_featured_video_settings_' . sanitize_title( $value['id'] ) . '_after' );
						}
						break;
					...
					case 'single_select_page':
						...
						<tr valign="top" class="single_select_page">
							...
							<td class="forminp">
								<?php echo str_replace( ' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'easy-featured-video' ) . "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", wp_dropdown_pages( $args ) ); // WPCS: XSS ok. ?> <?php echo $description; // WPCS: XSS ok. ?>
							</td>
						</tr>
						<?php
						break;
					...
					default:
						do_action( 'easy_featured_video_admin_field_' . $value['type'], $value );
						break;
				}
			}
		}
		...
		public static function save_fields( $options, $data = null ) {
			...
			foreach ( $options as $option ) {
				 ...
				 * @since 2.4.0
				 */
				$value = apply_filters( 'easy_featured_video_admin_settings_sanitize_option', $value, $option, $raw_value );
				 ...
				 * @since 2.4.0
				 */
				$value = apply_filters( "easy_featured_video_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
				 ...
				 */
				do_action( 'easy_featured_video_update_option', $option );
			}
		...
		public static function check_download_folder_protection() {
			...
			$downloads_url   = $upload_dir['basedir'] . '/easy_featured_video_uploads';
			$download_method = get_option( 'easy_featured_video_file_download_method' );
